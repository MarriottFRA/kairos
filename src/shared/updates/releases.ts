 ...
 *  dialog underneath it, open (each can be folded by hand). Nothing is ever
 *  removed from this list: a note is the only place a feature is announced, so
 *  deleting one un-announces it. A note may leave any of its three sections
 *  empty. */
export const UPDATE_NOTES: UpdateNote[] = [
  {
    ...
    new: [
      {
        title: "Books the movement",
        where: "Positions › Blocks › Multiplier › How it books",
        body:
          "A multiplier can book only how much its result MOVED from one month to the next. Build a balance with a chain of blocks — an end-of-service provision from salary × service, say — then add a multiplier on it and switch it to Books the movement. Every month books the balance less the previous worked month's, and the year adds up to closing minus opening. Each row's Opening balance is worked out for you — the same block run for last year, with last year's length of service and this year's pay before any increase — and a row with no balance in January starts from nothing, so a post that opens mid-year books its whole balance in its first month. Type a figure in the cell to use it instead (0 means nothing brought forward), or untick it on the block to type every row. A January pay rise restates the whole balance by itself, in January.",
      },
    ],
    improved: [],
    fixes: [],
